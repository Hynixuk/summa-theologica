(function () {
  ...
  // ---- AI search (server-side, via Groq) ----
  // Retrieval runs locally against the app's own search index; the answer
  // itself is generated by /api/search-answer, which calls Groq. Grounding the
  // model on retrieved excerpts keeps it to text actually in the corpus, and
  // it is instructed to answer briefly and point the reader at the full
  // passage rather than substitute for reading it.
  // (This replaced an in-browser WebLLM/WebGPU implementation, which required
  ...
  // feature still returns something useful if the API is unavailable.
  ...
  function renderAIAnswerShell() {
    ...
    aiAnswerEl.innerHTML =
      '<div class="ai-answer-label"><span class="ai-answer-spinner"></span><span id="aiAnswerStage">Thinking…</span></div>' +
      '<div id="aiAnswerBody" class="ai-answer-progress">Reading the closest passages and writing an answer.</div>';
  }
  ...
  function runAISearch(query) {
    ...
    fetch('/api/search-answer', {
      ...
      body: JSON.stringify({ query: query, context: context, allLowConfidence: allLowConfidence })
      ...
      renderAIAnswerText(data.answer || 'No answer generated.', sources, true);
    }).catch(function (err) {
      if (seq !== _aiQuerySeq) return;
      console.error('[AI search]', err);
      // Generation failed, but retrieval still found passages — showing those
      // is more useful to the reader than an error with nothing behind it.
      if (sources.length) {
        aiAnswerEl.hidden = false;
        aiAnswerEl.classList.remove('ai-answer-stale');
        renderExtractiveFallback(sources);
      } else {
        renderAIError('Could not generate an AI answer right now. Turn AI off to use keyword search instead.');
      }
    });
  }
